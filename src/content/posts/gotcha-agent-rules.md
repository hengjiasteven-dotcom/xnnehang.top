...
这篇聊的是 Agent 记忆管理中一个被低估的问题——有些东西不应该走"检索"路线，而是应该在 new session 启动时就全量加载。比如 Xnne 提到的 gotcha.md：一份绑在项目根目录的行为规则文件，记录那些 Agent 一开始就该知道、而不是等踩了坑才去学的东西。因为它们不适合走检索——query 不好拟定，召回无法保证 100%，不相关的 query 还可能引发行为冲突或幻觉。文章从真实痛点出发（同一个错误反复犯），讨论了 gotcha 的来源、落地方式、与 skill 的关系，以及"全量前置 vs 按需召回"的分界线在哪。如果你也在跟健忘的 Agent 搏斗，这篇或许能给个新思路。
:::
